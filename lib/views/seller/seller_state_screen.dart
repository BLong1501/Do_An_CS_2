  ...
  Future<void> _syncOldOrders(BuildContext context) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text("Hệ thống sẽ quét các xe 'Đã bán' trong quá khứ và tạo dữ liệu thống kê cho chúng. Bạn chỉ cần chạy cái này 1 lần."),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text("Hủy")),
          ElevatedButton(onPressed: () => Navigator.pop(ctx, true), child: const Text("Đồng bộ")),
        ],
      ),
    );
    ...
    try {
      ...
      final soldVehicles = await db.collection('vehicles')
      ...
      for (var doc in soldVehicles.docs) {
        ...
        final existingOrder = await db.collection('orders')
        ...
        if (existingOrder.docs.isEmpty) {
          ...
          Timestamp vehicleCreatedAt = vehicleData['createdAt'] ?? Timestamp.now();
      ...
      if (fixedCount > 0) {
        ...
        if (mounted) {
           ScaffoldMessenger.of(context).showSnackBar(
             const SnackBar(content: Text("Dữ liệu đã đồng bộ, không cần cập nhật thêm.")),
           );
        }
      }
 ...
 // --- LOGIC 1: TÍNH TỔNG SẢN PHẨM BÁN ĐƯỢC ---
  Future<void> _calculateProductsSold() async {
    ...
    try {
      ...
      // Log để debug
      print("Tìm thấy ${querySnapshot.docs.length} đơn hàng. Tổng sp tính được: $count");
  ...
  Future<void> _calculateMonthlyRevenue() async {
    ...
    try {
      ...
      final querySnapshot = await FirebaseFirestore.instance
          ...
          .where('sellerId', isEqualTo: uid)
          .where('status', isEqualTo: 'completed')
          .where('createdAt', isGreaterThanOrEqualTo: Timestamp.fromDate(startOfYear)) // Convert sang Timestamp rõ ràng
          .where('createdAt', isLessThanOrEqualTo: Timestamp.fromDate(endOfYear))
          .get();

      // Reset data về 0 hết
      Map<int, double> tempRevenue = {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Thống kê bán hàng", style: TextStyle(color: Colors.white)),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text("Tổng quan sản phẩm", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ...
              child: _isLoadingChart 
                ...
                : BarChart(
                    BarChartData(
                      ...
                      maxY: _monthlyRevenue.values.fold<double>(0.0, (p, c) => p > c ? p : c) * 1.2, // Tăng đỉnh biểu đồ lên chút
                      barTouchData: BarTouchData(
                        enabled: true,
                        touchTooltipData: BarTouchTooltipData(
                          getTooltipItem: (group, groupIndex, rod, rodIndex) {
                            return BarTooltipItem(
                              NumberFormat.compactCurrency(locale: 'vi', symbol: 'đ').format(rod.toY),
                      ...
                      titlesData: FlTitlesData(
                        ...
                        leftTitles: const AxisTitles(sideTitles: SideTitles(showTitles: false)), // Ẩn số bên trái cho gọn
